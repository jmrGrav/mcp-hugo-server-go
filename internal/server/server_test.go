...
func TestToolsListAuthenticatedReturnsTwentyTools(t *testing.T) {
	srv := mustOAuthServer(t)
	bearer := obtainBearerToken(t, srv)
	names := doMCPToolsList(t, srv, bearer)
	if len(names) != 20 {
		t.Fatalf("authenticated tools/list = %d tools, want 20; got %v", len(names), names)
	}
...
func TestLegacyMCPBearerBehavesLikeContentReadOverHTTP(t *testing.T) {
	...
	names := doMCPToolsList(t, srv, bearer)
	if len(names) != 20 {
		t.Fatalf("legacy mcp tools/list = %d tools, want 20; got %v", len(names), names)
	}
