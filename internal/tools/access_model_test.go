...
func TestVerifiedToolScopeMatrix(t *testing.T) {
	want := map[string]string{
		...
		"get_full_page_markdown":  "content.read",
		...
		"explain_site_structure":  "content.read",
		"get_site_health":         "content.read",
		"get_broken_links":        "content.read",
		"get_backlinks":           "content.read",
		"suggest_internal_links":  "content.read",
		"diff_page":               "content.read",
		"inspect_rendered_page":   "content.read",
		"validate_front_matter":   "content.read",
		...
		"generate_featured_image": "site.admin",
...
func TestCurrentAccessHierarchyStillMatchesDesignAnchor(t *testing.T) {
	...
	if got := len(readpkg.Defs()); got != 15 {
		t.Fatalf("content.read tool count = %d, want 15", got)
	}
	if got := len(writepkg.Defs()); got != 3 {
		t.Fatalf("content.write tool count = %d, want 3", got)
	}
	if got := len(adminpkg.Defs()); got != 7 {
		t.Fatalf("site.admin tool count = %d, want 7", got)
	}
