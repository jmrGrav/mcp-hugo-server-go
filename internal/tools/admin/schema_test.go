...
func TestAdminToolSchemasPresent(t *testing.T) {
	...
	for _, name := range []string{"build_site", "preview_build", "run_post_build_hooks", "generate_hero_image", "check_sri_versions", "list_previews", "revoke_preview", "revoke_all_previews", "inspect_preview_rendered"} {
		tool, ok := got[name]
		if !ok {
	...
	assertSchemaHasProperties(t, got["inspect_preview_rendered"], "inputSchema", "slug", "preview_id")
	assertSchemaHasProperties(t, got["inspect_preview_rendered"], "outputSchema.data", "inspection_scope", "preview_id", "preview_build", "preview_expires_at", "slug", "url", "lang", "output_path", "state", "status", "checks")
}
