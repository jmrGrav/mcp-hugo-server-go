...
func resolveUpdateFilePath(dir, slug, lang string) (string, error) {
	if path, ok, err := resolveBundleIndexFile(dir, slug, lang); ok || err != nil {
		return path, err
	}
	contentRoot := dir
	for range strings.Split(normalizeInputSlug(slug), "/") {
		contentRoot = filepath.Dir(contentRoot)
	}
	resolved, err := resolveExistingSource(contentRoot, slug, lang)
	if err != nil {
		return "", err
	}
	return resolved.SourcePath, nil
}

func resolveBundleIndexFile(dir, slug, lang string) (string, bool, error) {
	...
	if lang != "" {
		...
		return "", true, fmt.Errorf("not_found: no index.%s.md for slug %q; available: %s",
			lang, slug, strings.Join(bundleLangs(files), ", "))
	}
	if len(files) == 1 {
		return files[0], true, nil
	}
	return "", true, fmt.Errorf("ambiguous_language: page %q has multiple language files; specify lang (available: %s)",
		slug, strings.Join(bundleLangs(files), ", "))
}

func bundleLangs(files []string) []string {
	...
	for _, f := range files {
		...
		langs = append(langs, strings.TrimSuffix(strings.TrimPrefix(base, "index."), ".md"))
	}
	return langs
}
