...
type bundleContentLock struct{ held bool }

func (lock *bundleContentLock) acquire() error {
	if lock.held {
		return nil
	}
...
func (lock *bundleContentLock) release() {
	if lock.held {
...
func registerBundleLifecycleTools(s *mcp.Server, pg *security.PathGuard, idx *hugosite.SourceIndex, cfg config.Config, siteDB *db.DB, rt *writeRegisterRuntime) {
	registerCreateBundleTool(s, pg, idx, cfg, siteDB, rt)
	registerDeleteBundleTool(s, pg, idx, cfg, siteDB, rt)
}

func registerCreateBundleTool(s *mcp.Server, pg *security.PathGuard, idx *hugosite.SourceIndex, cfg config.Config, siteDB *db.DB, rt *writeRegisterRuntime) {
	mcp.AddTool(s, &mcp.Tool{Name: "create_bundle", Title: "Create multilingual bundle", Description: "Atomically create all translations of a new Hugo page bundle. Every page is validated before any file is written; a failure leaves no partial bundle. Each page may set `draft`, `description`, `featured_image`, and the same explicit `test_content: {ttl_hours?, owner?}` safety marker as create_page; test_content forces draft:true for that translation and returns its effective expiry in `data.test_content_expires_at`. dry_run previews the files without writing. Repeating the same non-dry-run request normally fails once any translation exists, but callers may provide `idempotency_key` to safely replay the exact same create attempt after a timeout or uncertain delivery — recoverable afterward via get_mutation_status(tool=\"create_bundle\"). Set `normalize_taxonomy_casing: true` (default off, #1191) to rewrite each page's submitted tags/categories that only differ in casing from a single existing spelling elsewhere in the index to that existing spelling, the same create_page/update_page behavior (#589) — applied per page, scoped to that page's own `lang` (a term is matched against existing forms in that language only, #604/#677); rewrites across every page are reported together in `data.taxonomy_casing_normalized`, and a term left untouched because the index already has two or more conflicting spellings for it in that page's language is reported in `data.taxonomy_casing_ambiguous` instead.", InputSchema: tools.MustSchema[createBundleInput](), OutputSchema: tools.MustSchema[bundleLifecycleOutput](), Annotations: &mcp.ToolAnnotations{ReadOnlyHint: false, DestructiveHint: fileutil.BoolPtr(false)}}, toolcontract.WrapTool(func(ctx context.Context, _ *mcp.CallToolRequest, in createBundleInput) (*mcp.CallToolResult, bundleLifecycleOutput, error) {
		return handleCreateBundle(ctx, in, pg, idx, cfg, siteDB, rt)
	}))
}

func handleCreateBundle(ctx context.Context, in createBundleInput, pg *security.PathGuard, idx *hugosite.SourceIndex, cfg config.Config, siteDB *db.DB, rt *writeRegisterRuntime) (*mcp.CallToolResult, bundleLifecycleOutput, error) {
...
func registerDeleteBundleTool(s *mcp.Server, pg *security.PathGuard, idx *hugosite.SourceIndex, cfg config.Config, siteDB *db.DB, rt *writeRegisterRuntime) {
	mcp.AddTool(s, &mcp.Tool{Name: "delete_bundle", Title: "Delete multilingual bundle", Description: "Atomically delete selected translations of a Hugo page bundle. `expected_revisions` must supply the current revision (from get_page/create_bundle/delete_bundle output) for every language listed, including for dry_run; the call fails invalid_params otherwise. All revisions are checked before the first unlink; a failure leaves the bundle unchanged. Pass every language to remove the bundle completely. Callers may provide `idempotency_key` to safely replay the exact same non-dry-run delete after a timeout or uncertain delivery — recoverable afterward via get_mutation_status(tool=\"delete_bundle\").", InputSchema: tools.MustSchema[deleteBundleInput](), OutputSchema: tools.MustSchema[bundleLifecycleOutput](), Annotations: &mcp.ToolAnnotations{ReadOnlyHint: false, DestructiveHint: fileutil.BoolPtr(true)}}, toolcontract.WrapTool(func(ctx context.Context, _ *mcp.CallToolRequest, in deleteBundleInput) (*mcp.CallToolResult, bundleLifecycleOutput, error) {
		return handleDeleteBundle(ctx, in, pg, idx, cfg, siteDB, rt)
	}))
}

func handleDeleteBundle(ctx context.Context, in deleteBundleInput, pg *security.PathGuard, idx *hugosite.SourceIndex, cfg config.Config, siteDB *db.DB, rt *writeRegisterRuntime) (*mcp.CallToolResult, bundleLifecycleOutput, error) {
