...
deploys and builds, and a same-day audit-and-fix pass against a live
external Codex agent audit of arleo.eu's public discovery surface — which
surfaced and closed a real production regression (isitagentready.com's
OAuth Protected Resource Metadata check) introduced by an earlier fix in
this same pass.

### Feat
- **BetterStack heartbeat monitoring on every deploy and build** (config: `heartbeat_hooks`): `run_post_build_hooks` gains an optional `status` param (`"success"`/`"fail"`, appends `/fail` to each configured URL) for manual reporting, and `build_site` now pings automatically and asynchronously — never blocking the build response — on both success and `OnBuildFailed`. `deploy.yml` gained a dedicated `heartbeat_report` job (`DEPLOY_HEARTBEAT_URL` secret) that always runs and checks both the `validate` and `deploy` job results, since an in-`deploy`-job step would never fire if `validate` failed and `deploy` got skipped.
- **`/openapi.json`**: OpenAPI 3.1.0 document covering `/register`, `/authorize`, `/token`, `/mcp`, `/agent/*`, and the `.well-known` discovery surface. Deliberately does not enumerate the MCP tool catalog as REST paths — it's dynamic and would go stale every release; `tools/list` over `/mcp` remains the source of truth. Closes the `api-catalog` linkset's dangling `application/openapi+json` reference.
- **`/health`**: minimal liveness probe (`{"status":"ok","version":...}`), referenced by `api-catalog`'s `status` link, which previously 404'd.
- **`auth.md`'s `/register`/`/token` `"returns"` field lists are now self-correcting**: `handleAuthMd` rewrites those two JSON arrays in-place via reflection over `oauth.RegistrationResponse`/`oauth.TokenResponse` before serving, regardless of what the hand-maintained source markdown says. `auth.md` lives in a separate content repo as prose and had drifted twice (a stale `client_secret` claim long after the server stopped issuing client secrets; a missing `scope` field on `/token` responses) — this closes that drift class structurally instead of relying on the next manual audit to catch it. `www.arleo.eu/auth.md` was also switched from proxying Hugo's static output to proxying the Go server directly, so both public hosts serve the same enforced copy instead of one being self-correcting and the other silently trusting the file.

### Fix
- **OAuth Protected Resource Metadata `"resource"` field mismatch on `www.arleo.eu`** (regression introduced same-day by an earlier fix in this pass, caught by a live isitagentready.com scan): replacing a hardcoded static JSON file with a `308` redirect to `mcp.arleo.eu` broke external validators that check `"resource"` against the origin they actually queried (RFC 9728) — the redirected response always named `mcp.arleo.eu/mcp` regardless of which host was asked. Fixed by reverse-proxying (not redirecting) `www.arleo.eu`'s `oauth-protected-resource` location to the Go server and making `buildProtectedResourceMeta` host-aware: it reflects the queried `Host` into `"resource"` only when that host exactly matches `cfg.SiteURL`'s hostname — never the issuer's own host, and never an arbitrary/spoofed `Host` header. The `/.well-known/oauth-protected-resource/mcp` path alias is explicitly excluded from this substitution, since RFC 9728 §3.1 fixes that alias's resource identity to `<issuer>/mcp` regardless of which host it's reached through.
...
### Docs
- `docs/public-url-topology.md`: every public arleo.eu URL, which layer serves it (MCP-dynamic / Hugo-static / OpenResty-only), and where its config actually lives — including the `sites-enabled`/`sites-available` divergence on the NUC for `www`/`mcp.arleo.eu` (editing `sites-available` for these two hosts has zero production effect, unlike every other vhost) and the SRI/CDN version cron's rebuild/deploy/commit behavior on hugo-vm.
- `check-sri-versions.sh` (hugo-vm, out-of-repo ops script) now commits and pushes its `sri.yaml`/`jsdelivr.yml` version bump to the `hugo-arleo.eu` mirror repo after a fully-verified auto-fix, instead of leaving the change live but uncommitted indefinitely. Only fires after success (never after a rollback); a git push failure on its own doesn't roll back an already-deployed fix, it raises a distinct non-blocking warning instead.
...
### Persistence (#1074 — durable content-state programme, completed this release)
- **Restart-safe build/publication manifest, first slice of #1077** (#1085): each completed `build_site`/`publish_changes` run persists its build ID, source/public tree fingerprints, status, and observation time to the optional site SQLite database (`publication_manifests` table). `get_runtime_status` restores the last completed build after a restart, explicitly labelled `last_build_persistence: "sqlite_manifest"`. The manifest remains derived operational state — Markdown/Git and `public/` stay canonical, and runtime checks still inspect current filesystem facts rather than trusting the stored record after an external edit. Does not make the content index authoritative, add a watcher, or change the `pages.slug UNIQUE` model.
- **Restart-safe idempotency journal, slice of #1078** (#1086): successful mutation results are persisted to SQLite (`mutation_journal` table, keyed by `(caller_key, tool, idempotency_key)`) when `db_path` is configured, so a same-key retry after a restart still replays correctly instead of re-executing. A journal read failure aborts the mutation attempt (fail-closed) at every one of the 8 mutation entry points rather than risking a double-apply.
- **Durable recovery journal for interrupted mutations and builds** (#1079, delivered across #1089/#1093/#1094/#1095): a `recovery_journal` table records every source/bundle write and build transition through `in_progress → file_written → committed`. On startup, `openSiteDB` reconciles any operation left mid-transition: a landed single-file write is detected by comparing the on-disk revision against the recorded before/after snapshot; a landed multilingual bundle write is detected the same way per file, and a genuinely mixed bundle (process death between two atomic renames) is rolled back to its durable pre-write state file-by-file. Reconciled mutations that already produced a result are replayed into the idempotency journal so a same-key retry after a restart returns the original response instead of re-executing. Build-only recovery is reconciled separately, only after a complete fresh SQLite source/public sync. A deterministic failure-injection test seam (`SetAfterFilesystemWriteHook`/`SetAfterRecoveryResultHook`) covers the boundary immediately after every mutation tool's atomic filesystem write.
- **Durable bundle rollback snapshots and ephemeral plan storage, slice of #1080** (#1093): `apply_bundle_plan`/`rollback_bundle` persist their before/after snapshots and caller-scoped TTL plan state to SQLite instead of process memory only, so a plan or rollback snapshot survives a restart within its TTL, scoped to the caller that created it.
- **Multilingual SQLite shadow migration, #1076** (#1090): a new `content_representations_v1` table records versioned `(source_key, lang, representation)` facts for both source and public content, derived from the existing indexes and reconciled without cutting any reader over yet. `get_runtime_status.data.content_index_shadow` reports aggregate-only counts, counterpart gaps, and a hashed (never raw-slug) mismatch fingerprint. Legacy slug-keyed reads remain authoritative for this release; this is the shadow-comparison groundwork for a later, separate cutover.
- **Durable multilingual build reconciliation, completing #1077** (#1096): `build_runs`/`build_pages` now record exact per-page filesystem fingerprints (`source_key + lang`) for every build, replacing the previous in-memory-only `BuildPending` signal as the source of truth once SQLite is configured. Startup and post-build reconciliation distinguish real source edits from public-output drift, correctly re-flag content deleted-then-restored with byte-identical bytes for republication, and persist the actual Hugo binary version used. `get_runtime_status.data.build_reconciliation` reports the aggregate drift counts.
- **Mutation journal retention observability, completing #1078** (#1092): expired mutation-journal entries are pruned transactionally on startup; `get_runtime_status.data.mutation_journal` reports active-entry count and the timestamp/count of the last successful sweep. A retention-maintenance failure is warn-only and never blocks startup or an in-flight mutation.
- **Restart-safe preview lease persistence, #1081** (#1091): preview ID, caller ownership, on-disk directory name, TTL, and lifecycle state persist to SQLite; bearer URL tokens and session cookies remain process-memory-only secrets and are never persisted, so a restored preview is listable/revocable but its old browser access is explicitly reported as `restart_invalidated`. Startup reconciliation removes expired, missing, or unsafe/orphaned preview directories with strict path-containment checks, and never follows a persisted path outside the managed preview root.
- **Post-write plan-consumption failures no longer misreport a live-retry path** (#1097): if `apply_content_plan`/`apply_bundle_plan`'s bookkeeping step that frees a consumed plan fails *after* the file write and recovery-journal record already succeeded, the response now downgrades to `partial_success` with a warning instead of a hard error that told the caller to retry — the file write is already durable and a live retry after that error actually hit `revision_conflict`, not a clean re-apply.

This closes out the durable content-state programme (#1074) for source/build/mutation/preview state. Legacy in-memory/slug-keyed reads remain authoritative pending a separate, later shadow-observation review; multi-principal concurrency and restart/recovery test coverage for the full matrix is tracked separately for v1.8.7 (#993).
...
- **Principal-less legacy access tokens fail closed** (#962): bearer validation now rejects persisted access tokens without a stable OAuth `principal` instead of silently falling back to a bearer-scoped quota identity. Clients must refresh or reauthorize; refresh-issued tokens retain their client principal. This closes the ambiguous legacy-token path without changing the token-bucket algorithm.
...
### Fix
- **`create_bundle` test-content parity with `create_page`/`update_page`** (#1038): a bundle page created with `test_content` set now correctly reports as a draft in `get_page`/`get_site_health` immediately after creation — the `SourceIndex` upsert previously never propagated the computed effective-draft state, so a `draft:false` bundle with `test_content` briefly appeared published/non-draft even though the on-disk file correctly forced it to draft.
- **`get_related_content`/`suggest_links` under-delivered results when combined with `language`/`one_per_source_key` filtering** (#1041): both tools truncated their scored candidate pool to the requested `limit` *before* applying multilingual filtering, so a same-source-key translation occupying a top-`limit` slot could push a real cross-language result out of the response entirely — down to zero results in the worst case. Both now over-fetch a larger candidate pool whenever either filter is active, then truncate to the real limit after filtering. `empty_reason` was also corrected to distinguish "no candidates scored well enough" from "candidates existed but were filtered out," so a caller isn't misled into loosening scoring when the real fix is dropping the filter.
- **`get_runtime_status` explains `source_ahead_of_public` with zero pending changes** (#1036): `data.source_ahead_reason` (`pending_mcp_changes`/`out_of_band_source_drift`/`generated_asset_drift`/`none`) and `data.publication_state` make explicit why the source is ahead of the public build even when the server has no pending pages of its own — previously a caller had to infer this from `git.dirty_classes` alone.
