...
- **New tool `rollback_change`** (#438, #340; **amends #379**): restores a page's source to a prior revision `apply_content_plan` itself snapshotted. #379 originally required a rollback target to be an immutable git commit, never "the state before the last apply" — that assumed the server would eventually make its own git commits, which turned out not to be true (this deployment's content checkout is host-managed, `baseline_mode: auto`, can be dirty, and the server has no git-commit code path). Amended (see #379's comment thread) to accept a server-held snapshot as a valid rollback target, deliberately scoped to revisions `apply_content_plan` itself produced and snapshotted — not arbitrary git history, and not a general "undo anything" mechanism. Guarded by the same `expected_revision` optimistic-concurrency check every other write tool uses, so it can never silently undo a newer, unrelated change; fails `snapshot_not_found` if no snapshot exists for the requested revision. Unlike a plan, a snapshot is not consumed on use (`IdempotentHint: true`). Requires `write`.
