...
- `update_page` - Update page (accepts optional `old_str`/`new_str` for an exact, unique, body-only snippet replacement instead of retransmitting the full `body`; zero or multiple matches fail closed; section indexes such as `_index.md` require `confirm_section_index:true` and this surgical patch form, including the homepage; also accepts optional `expected_bundle_revision` alongside `expected_revision` to additionally reject the write if a sibling translation or bundle-local asset changed since the caller last read the bundle; omitting it is a no-op — see #857/#1254/#1255)
