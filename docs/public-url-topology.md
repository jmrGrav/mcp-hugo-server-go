...
| `/auth.md` | Hugo-static, proxied for MCP identity | `proxy_pass` to hugo-vm:80 (Hugo's static output — `hugo-site/static/auth.md`), with a Lua header filter forcing `Content-Type: text/markdown`. Content is hand-maintained markdown, not generated from `internal/server/openapi.go` — the two can drift (they did: auth.md under-listed `/register`/`/token` response fields until 2026-08-22; `internal/oauth/oauth.go`'s `RegistrationResponse`/`TokenResponse` structs are the ground truth to check against). |
