...
| `require_delete_confirmation` | bool | `false` | When `true`, a non-dry-run `delete_page` call against a real (non-`test_content`) page with a source file is rejected unless the caller explicitly passes `confirm_delete_of_published_page: true`. This is on top of, not instead of, `delete_page`'s existing `expected_revision` requirement — that forces a prior read; this forces a distinct, named destructive-intent decision, giving the calling agent's own system prompt/guardrails a deliberate hook to obtain real human confirmation before setting it. Self-declared and unverifiable, like `declared_untrusted_derivation` — this server cannot confirm a human actually approved anything, only that the caller explicitly asserted intent rather than deleting by default. Pages carrying the `test_content` frontmatter marker (#661) and pages with no source file are exempt (no ceremony needed for disposable or already-public-only content). Off by default; a breaking change for any existing integration if enabled without updating its `delete_page` calls, so opt in deliberately. |
...
require_delete_confirmation: false # set true to require confirm_delete_of_published_page:true on delete_page for real (non-test_content) pages
