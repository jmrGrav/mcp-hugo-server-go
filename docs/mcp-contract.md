...
| `content_not_public` | no (deliberate) | overloaded across two meanings in this codebase — the legacy pre-#450 "draft/source-only content hidden from a public-safe reader" path, vs. the still-live "this diagnostics sub-feature is unavailable on the read tier" path. A single static hint would misclassify one of those meanings, so neither gets one |
