...
| `rate_limit_exceeded:` | Per-caller budget exceeded (`create_page`/`update_page`/`upload_page_asset`/`begin_asset_upload`/`commit_asset_upload` share one budget, `delete_page`/`delete_page_asset` share their own, separate one — see §6.3) |
...
| `begin_asset_upload` | structured | `data.upload_id`, `data.slug`, `data.source_key`, `data.filename`, `data.content_type`, `data.next_offset` (always `0`), `data.recommended_chunk_bytes` (advisory only), `data.max_asset_bytes`, `data.expires_at` (15-minute TTL) — starts a chunked upload for assets past `upload_page_asset`'s practical inline-base64 ceiling (#1196, split from #1190). `size_bytes` is checked against `max_asset_bytes` (10MiB) immediately, before any bytes transfer. Optional `sha256` (`"sha256:<64 hex>"`, same format as `upload_page_asset`'s `data.sha256`) is verified at `commit_asset_upload`, not here. The upload is scoped to the caller's own bearer token (the same isolation boundary `plan_content_change`/`rollback_change` snapshots use, #932/#934) — another caller's `upload_id` always reports `not_found`, never a distinguishable permission error. root `rate_limit_remaining` reports the shared `create_update_upload` budget; this call charges one token (#466) |
| `upload_asset_chunk` | structured | `data.upload_id`, `data.received_bytes`, `data.next_offset`, `data.size_bytes`, `data.complete` (bytes received == declared size; the asset is still not visible/validated until `commit_asset_upload` succeeds), `data.replayed` (true when this exact chunk — same `offset`, byte-identical content — was already received; a safe no-op retry, not an error). `offset` must equal the previous response's `next_offset`; a gap fails `out_of_order`, and different content at an already-received offset fails `chunk_conflict` rather than overwriting. Charges no rate-limit quota — `begin_asset_upload`'s `size_bytes` cap already bounds total transfer size regardless of chunk count |
| `commit_asset_upload` | structured | Same response shape as `upload_page_asset` (`data.status`, `data.slug`, `data.source_key`, `data.filename`, `data.path`, `data.content_type`, `data.size_bytes`, `data.sha256`, `data.duplicate_of?`, `data.dry_run?`) — finishes a `begin_asset_upload`/`upload_asset_chunk` sequence. Fails `incomplete_upload` if received bytes don't yet equal the declared `size_bytes`. The assembled bytes are validated through the exact same `validateAssetBytes` gate `upload_page_asset` uses (MIME sniff for png/jpg/jpeg/gif/webp, the strict structural allowlist parser for svg) — never a separate, potentially weaker check (#1202 extracted this specifically so both paths share it). A `sha256` mismatch (against either `begin_asset_upload`'s optional declared hash or this call's own optional `expected_sha256`) fails `sha256_mismatch` and discards the upload — the same `upload_id` cannot be retried after that. Never overwrites (`already_exists`, same as `upload_page_asset`). Supports `idempotency_key` for safe replay after a timeout. root `rate_limit_remaining` reports the shared `create_update_upload` budget; this call charges one token |
| `get_mutation_status` | structured | `data.tool`, `data.idempotency_key`, `data.status` (`"succeeded"`/`"unknown"`), `data.result?` — a read-only lookup of a prior `idempotency_key`-bearing `create_page`/`update_page`/`delete_page`/`upload_page_asset`/`delete_page_asset`/`commit_asset_upload` call, for recovering from a timeout/ambiguous response without resending the original payload; `data.result` (present only when `status: "succeeded"`) is the *entire* original response envelope (`success`/`data`/`errors`/`warnings`/`meta`), not just its inner `data` — the same shape a same-key/same-payload retry of the mutation tool itself would replay via its own idempotency cache. `status: "unknown"` covers still-in-flight, genuinely failed, expired (idempotency-key TTL, shared with the underlying idempotency cache; defaults to 15 minutes, configurable via `idempotency_ttl_seconds` in server config — a deployment-level setting only, never a per-call parameter, since a caller-supplied TTL could be used to shorten the window and evade duplicate-submission protection, #616), or never-attempted equally — only successful calls are ever recorded, so this is never proof of failure. Requires `content.write` (#586) |
| `get_rate_limits` | structured | `data.create_update_upload`/`data.destructive`, each `{remaining, limit, retry_after_seconds}` — the caller's current budget on both independent per-caller mutation quotas (`create_update_upload` shared by `create_page`/`update_page`/`upload_page_asset`/`begin_asset_upload`/`commit_asset_upload`/`apply_content_plan`/`rollback_change`; `destructive` shared by `delete_page`/`delete_page_asset`), reusing the exact same `callerLimiter`/`rateLimitRemaining` machinery those tools already populate their own `rate_limit_remaining` from (#378, #466) — a caller previously had no way to check quota before acting, only after a mutation call already reported `rate_limit_remaining`. `retry_after_seconds` is `0` if a call would succeed right now. Calling this tool is a pure read — it never itself consumes either quota. Requires `content.write`, the same trust level as the mutation tools whose quota it reports (#614) |
...
### 6.26 `mutation_journal.active_entries` Is a Retention Count, Not a Pending-Work Count (#1165)

`data.mutation_journal.active_entries` (populated only when SQLite is
configured) counts mutation results still retained by the idempotency
journal for replay — i.e. how many `create_page`/`update_page`/`delete_page`/
etc. results `RememberMutation` still holds so a retried request with the
same `idempotency_key` can safely replay the original result instead of
re-executing. It says nothing about whether the underlying content is still
unpublished: an entry stays until its retention window prunes it, regardless
of whether that page was later published, edited again, or deleted. A
freshly-deployed, fully-published site can legitimately report a large
nonzero `active_entries`.

For "how much unpublished work exists," read `data.publication_safety` /
...
### 6.28. `get_capabilities.data.tool_catalog.tool_registry_digest`: Tool-Poisoning / Rug-Pull Detection (#1225)

`tool_names_revision` (#1175, [§6.22](#6-22-exposure-profiles-profile-on-the-mcp-endpoint-1137))
only hashes tool *names*, sorted — it detects a tool being added or
removed, but not an existing tool's description or input/output schema
being silently edited after a client already approved it. That gap is
exactly the "rug pull" / tool-poisoning attack: a tool that looked safe
when a human or agent reviewed and approved it gets its description
rewritten later (e.g. to embed hidden instructions, or to widen what a
parameter accepts) without the tool's name ever changing, so a client
that only pins names never notices.
