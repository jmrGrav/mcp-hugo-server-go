...
`score_breakdown` deliberately covers only `frontmatter`, `taxonomy`, and
`title_shape` — the categories this server computes a real signal for today.
It omits `links`/`rendering`/`publication` placeholders an earlier proposal
sketched; publishing a fabricated 100 for a category with no underlying check
would be more misleading than omitting it. Whether `get_broken_links` volume
should become a scored category too is an open question (#1105), tracked
separately from this title-shape addition.
