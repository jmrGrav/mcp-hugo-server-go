...
| `apply_content_plan` | structured | `data.status`, `data.plan_id`, `data.slug`, `data.dry_run?`, `data.before_revision`, `data.after_revision?`, `data.validation`, `data.warning?`, `data.state?`; root `rate_limit_remaining` on the shared create/update/upload quota, on both success and error (#466, #510). Takes only `plan_id` (+ `idempotency_key`/`dry_run`) — no body/title/tags resent, apply writes exactly what the plan already computed. Fails `plan_not_found` if `plan_id` is unknown, already applied, or its TTL expired; fails `revision_conflict` if the page changed since the plan was created. **A plan is single-use**: a real (non-`dry_run`) apply attempt consumes it whether the write succeeds or fails; `dry_run` re-verifies without consuming it. Deliberately writes source only — no build/publish fields; that is `publish_changes`'s layer (#340), a separate, later, explicitly-confirmed step. `rollback_change` (#340) remains unbuilt: this deployment has no controlled git-commit capability, and a server-side content snapshot would violate #379's invariant that only a real commit is a valid rollback target, never "the state before the last apply" (#438) |
