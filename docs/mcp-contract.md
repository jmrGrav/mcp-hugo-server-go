...
| `get_runtime_status`      | structured | `data.release_version`, `data.commit`, `data.hugo`, `data.git`, `data.site`, `data.degraded`; when disposable `test_content` has expired, `data.site.overdue_test_content[]` exposes a machine-readable cleanup/advisory list (`slug`, `owner?`, `expires_at`, `overdue_seconds`, `reason`) without requiring a build/publish call first (#757) |
